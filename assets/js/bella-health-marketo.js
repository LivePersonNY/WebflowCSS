...
function setCookie(name, value, days) {
	...
	document.cookie = name + "=" + value + expires + "; path=/; domain=bella.health;";
}
...
var lpindex = lsRef.indexOf('bella.health');
...
if (leadSourceCookie === '') {
	...
	} else if (lsRef.indexOf('go.bella.health') >= 0 || lsRef.indexOf('mkto-g0178.com') >= 0) {
		setCookie('lp-leadSource', 'Email', 1);
		leadSourceCookie = 'Email';
...
var splitFullName = function (a, b, c, form) {
	...
	return;
	//Initial Values
	if (form.getValues().wholeName.length < 2 && form.getValues().LastName.length > 2 && form.getValues().FirstName.length > 2) {
